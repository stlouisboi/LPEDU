...
const AdminDashboardHome = () => {
  ...
  useEffect(() => {
    const checkHealth = async () => {
      ...
      try {
        // Simple light check - attempt to fetch a non-existent doc to trigger a connection
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted mt-1">Welcome back. Here is what's happening today.</p>
        </div>
      ...
      {dbStatus === 'not-found' && (
        <div className="bg-red-50 dark:bg-red-950/20 p-8 rounded-[2.5rem] border border-red-200 dark:border-red-900/50 flex flex-col items-center text-center gap-6 animate-in zoom-in-95 duration-500 shadow-xl">
          ...
          <div>
            <h3 className="text-2xl font-bold text-red-800 dark:text-red-400 font-serif">Firestore Database (default) Missing</h3>
            <p className="text-red-700/80 dark:text-red-300/80 mt-2 max-w-xl mx-auto leading-relaxed">
              Your Firebase project <strong>launchpathedu-426fb</strong> exists, but the Cloud Firestore database instance has not been created yet. You must initialize it in the Google Cloud console.
            </p>
          </div>
          <a 
            href="https://console.cloud.google.com/datastore/setup?project=launchpathedu-426fb" 
            target="_blank" 
            rel="noreferrer" 
        ...
        <div className="bg-amber-50 dark:bg-amber-950/20 p-8 rounded-[2.5rem] border border-amber-200 dark:border-amber-900/50 flex flex-col md:flex-row items-center justify-between gap-6 animate-in zoom-in-95 duration-300 shadow-lg">
          <div className="flex items-center space-x-5">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700/80 dark:text-amber-300/80 mt-1">The API is not enabled for project <span className="font-mono bg-white/50 px-1 rounded">launchpathedu-426fb</span>.</p>
            </div>
          </div>
          <a 
            href="https://console.cloud.google.com/apis/library/firestore.googleapis.com?project=launchpathedu-426fb" 
            target="_blank" 
            rel="noreferrer" 
