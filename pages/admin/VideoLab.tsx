  ...
  const generateAIVideo = async () => {
    ...
    try {
      ...
      while (!op.done) {
        ...
        // Use a fresh GoogleGenAI instance for every polling call to ensure latest API key is used
      ...
      if (e.message?.includes("Requested entity was not found")) {
        setHasApiKey(false);
        alert("Re-authorization required. Please authorize your Studio key again.");
      } else alert("Neural synthesis failed. Check your connection or visual directives.");
    } finally {
        ...
        {videos.map((vid) => (
          <div key={vid.id} className="bg-white dark:bg-surface-dark rounded-[2.5rem] border border-border-light dark:border-border-dark overflow-hidden shadow-sm group relative hover:shadow-xl transition-all">
            <div className={`relative ${vid.aspectRatio === '9:16' ? 'aspect-[9/16]' : 'aspect-video'} bg-black`}>
              ...
              <button onClick={async () => { if(window.confirm("Purge asset?")) { await deleteDoc(doc(db, "generatedVideos", vid.id)); if(vid.storagePath) try { await deleteObject(ref(storage, vid.storagePath)); } catch(e){} } }} className="absolute top-4 right-4 p-3 bg-red-500 text-white rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity z-20 shadow-lg"><Trash2 size={18} /></button>
            </div>
            <div className="p-8">
              <span className="bg-authority-blue/10 text-authority-blue dark:text-signal-gold px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">{vid.moduleId === 0 ? 'Ground 0' : `Module ${vid.moduleId}`}</span>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <input value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} placeholder="Production metadata..." className="w-full bg-slate-50 dark:bg-gray-800 border dark:border-border-dark border-slate-100 rounded-3xl px-8 py-5 font-bold" />
                  </div>
                </div>
                ...
                <div className="bg-slate-50 dark:bg-gray-900 rounded-[3rem] p-8 border border-slate-100 dark:border-border-dark relative flex flex-col justify-center min-h-[400px] shadow-inner">
                  {productionMode === 'ai' ? (
                    <div className="space-y-6">
                       ...
                       <>
                          <textarea rows={5} value={formData.prompt} onChange={(e) => setFormData({...formData, prompt: e.target.value})} placeholder="Describe cinematic visualization..." className="w-full bg-white dark:bg-gray-800 border dark:border-border-dark rounded-[2rem] p-6 text-sm font-bold shadow-sm" />
