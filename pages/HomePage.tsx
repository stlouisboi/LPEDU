...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="animate-in fade-in duration-700 relative overflow-x-hidden bg-white dark:bg-primary-dark font-sans text-authority-blue">
      ...
      <section className="py-32 bg-white dark:bg-primary-dark">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            ...
            <div id="pillars" className="lg:col-span-6 bg-slate-50 dark:bg-surface-dark p-10 md:p-12 rounded-[3.5rem] border border-slate-200 dark:border-border-dark shadow-sm">
      ...
      {/* NEW: THE 16 DEADLY SINS CTA */}
      <section className="py-24 bg-slate-50 dark:bg-surface-dark border-y border-slate-200 dark:border-border-dark">
        <div className="max-w-5xl mx-auto px-6 text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-red-100 dark:bg-red-900/20 text-red-600 mb-8 rounded-3xl shadow-sm">
            <FileWarning size={32} />
          </div>
          <h2 className="text-3xl md:text-5xl font-black font-serif text-authority-blue dark:text-white uppercase tracking-tight mb-6">
            The 16 Deadly Sins: <br/><span className="text-red-600 italic">Your Audit Risk Exposure.</span>
          </h2>
          <p className="text-xl text-text-muted dark:text-text-dark-muted font-medium max-w-3xl mx-auto leading-relaxed mb-10">
          ...
          <button 
            onClick={() => document.getElementById('deadly-sins-details')?.scrollIntoView({ behavior: 'smooth' })}
            className="bg-authority-blue text-white px-12 py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-[#152a44] transition-all shadow-xl flex items-center mx-auto"
          >
            <span>See the 16 Deadly Sins</span>
            <ArrowRight className="ml-3 w-4 h-4" />
          </button>
        </div>
      </section>

      {/* NEW: THE 16 DEADLY SINS CONTENT SECTION */}
      <section id="deadly-sins-details" className="py-32 bg-white dark:bg-primary-dark overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <div className="lg:col-span-4 lg:sticky lg:top-48">
              <h2 className="text-4xl font-black font-serif text-authority-blue dark:text-white uppercase tracking-tight leading-none mb-6">
                Critical <br/><span className="text-red-600">Exposure</span> <br/>Points.
              </h2>
              <p className="text-lg text-text-muted font-medium leading-relaxed mb-8">
                The FMCSA classifies certain violations as "Acute" or "Critical." These are the system failures that trigger immediate intervention.
              </p>
              <div className="p-6 bg-slate-50 dark:bg-surface-dark rounded-[2rem] border border-slate-100 dark:border-border-dark space-y-4">
                 <div className="flex items-center space-x-3 text-red-600">
                    <ShieldAlert size={18} />
                    <span className="text-[10px] font-black uppercase tracking-widest">Enforcement Priority</span>
                 </div>
                 <p className="text-xs font-bold text-text-muted leading-relaxed">
                   Identifying these sins is the first step toward building the protective documentation required for survival.
                 </p>
              </div>
            </div>
            ...
            <div className="lg:col-span-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={i} className="group p-6 bg-white dark:bg-surface-dark border border-slate-100 dark:border-border-dark rounded-2xl hover:border-red-200 dark:hover:border-red-900/30 transition-all flex items-start space-x-4 shadow-sm hover:shadow-md">
                    <span className="text-xs font-black text-slate-300 dark:text-slate-700 group-hover:text-red-500 transition-colors pt-1">0{i+1 > 9 ? i+1 : '0'+(i+1)}</span>
                    <p className="text-sm font-bold text-authority-blue dark:text-text-dark-muted leading-snug">{sin}</p>
                  </div>
                ))}
              </div>
              <div className="mt-12 p-8 bg-authority-blue text-white rounded-[2.5rem] shadow-xl relative overflow-hidden">
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="max-w-md">
                    <h3 className="text-xl font-bold font-serif mb-2">Systems Presence Prevents These Sins.</h3>
                    <p className="text-sm opacity-70 leading-relaxed">The LaunchPath standard builds the documentation backbone that ensures these 16 sins never appear in your operational record.</p>
                  </div>
                  <Link to="/readiness" className="bg-white text-authority-blue px-8 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-signal-gold transition-all shrink-0">
                    Get Protected
                  </Link>
                </div>
              </div>
