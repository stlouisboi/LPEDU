...
const AdmissionTerminalV42: React.FC<AdmissionTerminalV42Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-0 sm:p-4 bg-[#002244]/98 backdrop-blur-xl animate-in fade-in duration-500 overflow-y-auto">
      <div className="bg-[#FAF9F6] dark:bg-primary-dark w-full max-w-5xl sm:rounded-[3rem] md:rounded-[4rem] overflow-hidden shadow-2xl border-0 sm:border-[8px] md:border-[10px] border-white/5 relative flex flex-col lg:flex-row min-h-screen sm:min-h-0 sm:max-h-[95vh] overflow-y-auto custom-scrollbar">
        
        <div className="w-full lg:w-[38%] bg-[#002244] p-8 sm:p-12 lg:p-14 text-white flex flex-col justify-between relative border-b lg:border-b-0 lg:border-r border-white/10 shrink-0">
          <div className="space-y-6 sm:space-y-12 relative z-10">
            <div className="flex justify-between items-center lg:block lg:space-y-12">
              <Logo light={true} className="h-7 sm:h-10 w-auto" />
              <button onClick={onClose} className="lg:hidden p-2 bg-white/5 rounded-lg text-white/40 hover:text-white transition-colors" aria-label="Close Terminal"><X size={24} /></button>
            </div>
            
            <div className="space-y-2 sm:space-y-4">
              <h3 className="text-[10px] sm:text-xs font-black uppercase tracking-[0.4em] text-[#C5A059]">Registry Terminal</h3>
              <p className="text-3xl sm:text-5xl font-black font-serif uppercase tracking-tighter">V4.2</p>
        ...
        <div className="flex-grow p-8 sm:p-12 lg:p-20 relative bg-white dark:bg-primary-dark">
          <button 
            onClick={onClose}
            className="hidden lg:flex absolute top-8 right-8 p-2.5 bg-slate-100 dark:bg-gray-800 rounded-full hover:text-red-500 transition-all group"
          ...
          <form onSubmit={handleRegistrySubmit} className="space-y-8 sm:space-y-12">
            <div className="space-y-2 sm:space-y-3">
              <h2 className="text-2xl sm:text-4xl font-black font-serif uppercase tracking-tight text-[#002244] dark:text-white leading-none">Admission Sequence</h2>
              <p className="text-[9px] sm:text-[11px] font-black uppercase tracking-[0.4em] text-slate-400">Master Identity Profile</p>
            </div>
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
                <div className="space-y-2">
                  ...
                  <input required value={formData.carrierName} onChange={e => setFormData({...formData, carrierName: e.target.value})} placeholder="Carrier Entity" className="w-full bg-slate-50 dark:bg-gray-800 border-2 border-slate-100 dark:border-border-dark rounded-xl sm:rounded-2xl px-6 py-4 sm:py-5 outline-none font-black text-sm dark:text-white uppercase shadow-inner" />
                </div>
                <div className="space-y-2">
                  <label className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">DOT Number (Optional)</label>
                  <input value={formData.dotNumber} onChange={e => setFormData({...formData, dotNumber: e.target.value})} placeholder="DOT #######" className="w-full bg-slate-50 dark:bg-gray-800 border-2 border-slate-100 dark:border-border-dark rounded-xl sm:rounded-2xl px-6 py-4 sm:py-5 outline-none font-black text-sm dark:text-white shadow-inner" />
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 sm:pt-6">
              <button 
                ...
                className="w-full bg-[#002244] text-white py-5 sm:py-8 rounded-xl sm:rounded-[2.5rem] font-black uppercase tracking-[0.4em] text-[10px] sm:text-xs shadow-2xl hover:bg-[#0c1a2d] transition-all active:scale-[0.98] border-b-4 sm:border-b-8 border-slate-950 flex items-center justify-center disabled:opacity-50"
              >
                {isSubmitting ? (
                  <div className="flex items-center">
                    <Loader2 className="animate-spin mr-3" size={20} sm:size={24} />
