...
def _build_reach_email(result: str, total_score: int, scores: "REACHCategoryScores", email: str) -> tuple[str, str]:
    ...
    if result == "GO":
        subject = f"REACH Result: GO — You're cleared to move forward."
        html = header_base + f"""
        ...
        subject = f"REACH Result: WAIT — Address these gaps before proceeding."
        html = header_base + f"""
        ...
        subject = f"REACH Result: NO-GO — Your current position requires intervention."
        html = header_base + f"""
