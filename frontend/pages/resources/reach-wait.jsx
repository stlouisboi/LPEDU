...
export default function ReachWaitStatus() {
  useEffect(() => {
    track('reach_wait_page_view');
  }, []);

  return (
    <>
      <Head>
        <title>WAIT Status | LaunchPath REACH</title>
      ...
      <div style={{ background: C.bg, minHeight: '100vh', fontFamily: S.body, padding: '0' }}>
        ...
        <div style={{ borderBottom: `1px solid ${C.border}`, padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontFamily: S.mono, fontSize: '9px', letterSpacing: '0.12em', color: C.amber, textTransform: 'uppercase' }}>
            STATUS: WAIT
          </span>
        </div>
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto', padding: '72px 32px' }}>
          ...
          {/* Outcome label */}
          <p style={{ fontFamily: S.mono, fontSize: '10px', fontWeight: 700, letterSpacing: '0.22em', color: C.amber, textTransform: 'uppercase', margin: '0 0 24px' }}>
            LP-STATUS: REACH — CORRECTIVE PAUSE / WAIT
          </p>
          ...
          <p style={{ fontFamily: S.body, fontSize: '15px', color: C.text, lineHeight: 1.8, margin: '0 0 16px' }}>
            REACH identified conditions that need to be corrected before moving forward is appropriate. Proceeding with those conditions unaddressed creates exposure that becomes harder to correct once authority is active.
          </p>
          ...
          {[
            ...
            { label: 'Repair before re-entering', body: 'Do not return to REACH until the conditions that caused the WAIT result have been meaningfully addressed.' },
            { label: 'Return when the foundation is stronger', body: 'When you re-enter, REACH will reassess from the beginning. This is a fresh evaluation, not an automatic continuation.' },
          ].map((item) => (
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${C.border}`, borderLeft: `3px solid rgba(197,160,89,0.40)`, padding: '24px 28px', marginBottom: '40px' }}>
            ...
            {[
              ...
              'When you return, a fresh REACH assessment is required.',
            ].map((line) => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <Link
              href="/reach-diagnostic?return=wait"
              data-testid="reach-wait-return-cta"
              onClick={() => track('reach_wait_return_click')}
              style={{
            ...
            >
              Return to REACH Later &rarr;
            </Link>
            <p style={{ fontFamily: S.body, fontSize: '12px', color: C.textDim, lineHeight: 1.6, margin: 0 }}>
              This routes to a fresh REACH entry. Prior WAIT metadata is preserved for continuity.
            </p>
          </div>
