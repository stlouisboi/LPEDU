...
export default function Ground0NoGoStatus() {
  const [holdConfirmed, setHoldConfirmed] = useState(false);

  useEffect(() => {
    track('ground0_nogo_page_view');
  }, []);

  const handleHold = () => {
    track('ground0_nogo_hold_click');
  ...
  return (
    <>
      <Head>
        <title>NO-GO Status | LaunchPath Ground 0</title>
      ...
      <div style={{ background: C.bg, minHeight: '100vh', fontFamily: S.body }}>
        ...
        <div style={{ maxWidth: '680px', margin: '0 auto', padding: '72px 32px' }}>
          ...
          <p style={{ fontFamily: S.body, fontSize: '15px', color: C.text, lineHeight: 1.8, margin: '0 0 16px' }}>
            Ground 0 exists to mark that line clearly — before forward movement creates damage that is harder to reverse than the pause would have been.
          </p>
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${C.border}`, borderLeft: `3px solid rgba(248,113,113,0.30)`, padding: '24px 28px', marginBottom: '40px' }}>
            ...
            {[
              ...
              'A fresh Ground 0 assessment will be required before any re-entry.',
            ].map((line) => (
          ...
          {holdConfirmed ? (
            <div
              data-testid="nogo-hold-confirmed"
            ...
            >
              ...
              <p style={{ fontFamily: S.body, fontSize: '14px', color: C.textMuted, lineHeight: 1.7, margin: 0 }}>
                You remain on the list for future follow-up. If circumstances change later, Ground 0 can be revisited from a different starting point.
              </p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <button
                data-testid="nogo-remain-btn"
                onClick={handleHold}
                style={{
