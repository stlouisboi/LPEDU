        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "1px", background: "var(--border)" }}>
          {STEPS.map((s) => (
            <div key={s.num} data-testid={`step-${s.num}`} style={{
              ...
              <h3 style={{ fontFamily: "'Manrope', sans-serif", fontWeight: 700, fontSize: "1.05rem", color: "var(--text)" }}>{s.name}</h3>
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.875rem", color: "var(--text-muted)", lineHeight: 1.7, flex: 1 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
