  ...
  if (phase === "landing") return wrap(
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "4rem", alignItems: "start" }} className="readiness-grid">
      <div>
        ...
        <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "1rem", color: "var(--text-muted)", lineHeight: 1.8, marginBottom: "0.75rem" }}>
          Before you file for authority, identify the exposure points that could end your operation in the first 90 days.
        </p>
        <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.875rem", color: "var(--text-subtle)", marginBottom: "2rem" }}>
        ...
        <div style={{ borderLeft: "2px solid var(--orange)", paddingLeft: "1.25rem", marginBottom: "2.5rem" }}>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.9rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.25rem" }}>Accuracy over ambition.</p>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.85rem", color: "var(--text-subtle)" }}>Your result — GO, WAIT, or STOP — is only useful if it reflects reality.</p>
        </div>
        <button
        ...
        <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.75rem", color: "var(--text-subtle)", marginTop: "0.75rem" }}>Free. No commitment required.</p>
      </div>

      {/* Section panel */}
      <div style={{ border: "1px solid var(--border)", background: "var(--bg-2)", padding: "1.5rem" }}>
        <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.6rem", color: "var(--text-subtle)", letterSpacing: "0.12em", marginBottom: "1.25rem", paddingBottom: "1rem", borderBottom: "1px solid var(--border)" }}>
        ...
        {QUESTIONS.map((q, i) => (
          <div key={i} style={{ display: "flex", gap: "0.75rem", padding: "0.6rem 0", borderBottom: i < 6 ? "1px solid var(--border)" : "none" }}>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.6rem", color: "var(--orange)", letterSpacing: "0.08em" }}>0{i + 1}</span>
            <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.8rem", color: "var(--text-subtle)" }}>{q.section.split(" — ")[1]}</span>
          </div>
        ))}
        <div style={{ marginTop: "1.25rem", paddingTop: "1.25rem", borderTop: "1px solid var(--border)" }}>
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.6rem", color: "var(--text-subtle)", letterSpacing: "0.1em", marginBottom: "0.75rem" }}>DECISION OUTCOMES</p>
          {[["GO", "var(--orange)", "Proceed to admission"], ["WAIT", "#d97706", "Remediate, then return"], ["STOP", "#ef4444", "Do not proceed"]].map(([label, color, desc]) => (
            <div key={label} style={{ display: "flex", gap: "0.75rem", alignItems: "baseline", padding: "0.4rem 0" }}>
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.65rem", color, fontWeight: 700, minWidth: 36 }}>{label}</span>
              <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", color: "var(--text-subtle)" }}>{desc}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ marginBottom: "2.5rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
            <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.65rem", color: "var(--orange)", letterSpacing: "0.12em" }}>
              QUESTION {currentQ + 1} OF 7
            </p>
            <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.65rem", color: "var(--text-subtle)" }}>
          ...
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.58rem", color: "var(--text-subtle)", letterSpacing: "0.1em", marginTop: "0.5rem" }}>
          ...
          {q.options.map((opt) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ flex: 1, fontFamily: "'Inter', sans-serif", fontSize: "0.95rem", color: "var(--text)", lineHeight: 1.5 }}>{opt.text}</span>
                {opt.risk && (
                  <span style={{
  ...
  if (phase === "email-gate") return wrap(
    <div style={{ maxWidth: 600, margin: "2rem auto" }}>
      ...
      <form onSubmit={submitEmail} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
        <div>
          <label style={{ display: "block", fontFamily: "'Inter', sans-serif", fontSize: "0.7rem", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-subtle)", marginBottom: "0.5rem" }}>
            Email Address
          </label>
          <input
        ...
        <label style={{ display: "flex", gap: "0.875rem", alignItems: "flex-start", cursor: "pointer", padding: "1rem", border: "1px solid var(--border)", background: "var(--bg-2)" }}>
          ...
          />
          <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.85rem", color: "var(--text-subtle)", lineHeight: 1.6 }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1.25rem", marginBottom: "2rem", animation: "heroEnter 0.5s ease both" }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.65rem", color: "var(--text-subtle)", marginTop: "0.35rem" }}>Score: {totalScore} of 28</p>
          </div>
        </div>

        <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.95rem", color: "var(--text-muted)", lineHeight: 1.85, marginBottom: "2.5rem", maxWidth: 560 }}>
          {r.body}
        </p>
        ...
        {flagged.length > 0 && (
          <div style={{ border: "1px solid var(--border)", marginBottom: "2.5rem" }}>
            <div style={{ padding: "0.875rem 1.25rem", borderBottom: "1px solid var(--border)", background: "var(--bg-2)" }}>
              <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.6rem", color: "var(--text-subtle)", letterSpacing: "0.1em" }}>FLAGGED EXPOSURE POINTS</p>
            </div>
            {flagged.map((q, i) => {
              ...
              return (
                <div key={i} style={{ display: "flex", gap: "1rem", alignItems: "flex-start", padding: "0.875rem 1.25rem", borderBottom: i < flagged.length - 1 ? "1px solid var(--border)" : "none" }}>
                  <span style={{
                    ...
                    padding: "0.15rem 0.5rem", fontFamily: "'JetBrains Mono', monospace", fontSize: "0.58rem", flexShrink: 0,
                  }}>{ans.risk}</span>
                  <div>
                    <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.8rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.2rem" }}>{q.section}</p>
                    <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.8rem", color: "var(--text-subtle)" }}>Selected: {ans.text}</p>
                  </div>
                </div>
              );
            })}
        ...
        <button
          ...
          style={{ marginTop: "1.5rem", background: "none", border: "none", fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", color: "var(--text-subtle)", cursor: "pointer", padding: 0, textDecoration: "underline" }}
        >
          Restart assessment
        </button>
      </div>
