    ...
    <div style={{ background: "#FFFFFF", border: "1px solid rgba(11,22,40,0.10)", marginBottom: 12 }}>
      <button onClick={onToggle} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 24px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
        <div>
          ...
          <p style={{ fontFamily: "'Newsreader', 'Playfair Display', serif", fontWeight: 700, fontSize: 18, color: NAVY, margin: 0, letterSpacing: "-0.01em" }}>{title}</p>
        </div>
...
export default function TCOCalculatorPage() {
  ...
  const [gateState, setGateState] = useState("idle"); // idle | submitting | unlocked | error
  ...
  const handleGateSubmit = async (e) => {
    ...
    try {
      const r = await fetch(`${API}/api/tools/tco-subscribe`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: gateEmail, name: gateName }),
      });
      if (r.ok) setGateState("unlocked");
    ...
    <div style={{ background: BG, minHeight: "100vh" }}>
      ...
      <div style={{ background: NAVY, borderBottom: "1px solid rgba(212,144,10,0.20)" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "4rem 1.5rem 3rem" }}>
          ...
          <h1 style={{ fontFamily: "'Newsreader', 'Playfair Display', serif", fontWeight: 700, fontSize: "clamp(1.75rem, 4vw, 2.5rem)", color: "#FFFFFF", letterSpacing: "-0.025em", marginBottom: 10 }}>True Cost of Ownership</h1>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "2.5rem 1.5rem 6rem" }}>
          ...
          {/* Results Panel — gated behind email capture */}
          {gateState !== "unlocked" ? (
            <div data-testid="tco-email-gate" style={{ background: NAVY, padding: "28px 28px 24px", marginTop: 20, marginBottom: 12 }}>
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "rgba(212,144,10,0.65)", margin: "0 0 8px" }}>YOUR RESULTS ARE READY</p>
              <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, color: "rgba(255,255,255,0.70)", lineHeight: 1.7, margin: "0 0 20px" }}>
                Enter your email to unlock your full cost-per-mile breakdown and 30% margin target.
              </p>
              <form onSubmit={handleGateSubmit} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <input
                  data-testid="tco-gate-name"
                  type="text"
                  placeholder="Your name (optional)"
                  value={gateName}
                  onChange={e => setGateName(e.target.value)}
                  style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, padding: "11px 14px", border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.06)", color: "#fff", outline: "none" }}
                />
                <input
                  data-testid="tco-gate-email"
                  type="email"
                  placeholder="Email address *"
                  required
                  value={gateEmail}
                  onChange={e => setGateEmail(e.target.value)}
                  style={{ fontFamily: "'Inter', sans-serif", fontSize: 14, padding: "11px 14px", border: "1px solid rgba(255,255,255,0.20)", background: "rgba(255,255,255,0.06)", color: "#fff", outline: "none" }}
                />
                <button
                  data-testid="tco-gate-submit"
                  type="submit"
                  disabled={gateState === "submitting"}
                  style={{ fontFamily: "'Inter', sans-serif", fontWeight: 700, fontSize: 13, letterSpacing: "0.08em", textTransform: "uppercase", background: GOLD, color: NAVY, border: "none", padding: "13px 24px", cursor: "pointer", transition: "background 0.2s" }}
                >
                  {gateState === "submitting" ? "Unlocking…" : "Unlock My Results →"}
                </button>
          ...
          {/* Save + Link to LP-TOOL-002 — only visible after unlock */}
          {gateState === "unlocked" && (
